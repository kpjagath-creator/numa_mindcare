...
| `ClinicalNoteAmendment` | Clinical notes | Append-only, added 2026-08-30 (CLN-07) — never mutates its parent `ClinicalNote` |
...
- **Patient Timeline → Scheduling and Clinical Notes (read-only), added 2026-08-30 (PAT-10).** `patientTimelineService.ts` (Patients domain, `GET /patients/:id/timeline`) reads `PatientStatusLog` (its own domain), `TherapySession`, and `ClinicalNote` (via a `session: { patientId }` filter) in three parallel queries and composes them into one ordered, typed list — the same kind of intentional cross-domain read Analytics already does, not a new event table or generic composition framework. Writes nothing.
...
4. **Scheduling conflict rules:** a patient or a therapist cannot have two overlapping sessions unless one is `cancelled`, `rescheduled`, or `no_show` (`CONFLICT_EXCLUDED_STATUSES` in `therapySessionsService.ts`). Enforced at the application layer via a `findFirst` overlap query inside the same transaction as the write, for a fast/friendly rejection message. **As of 2026-08-30 (SCH-05), this is additionally enforced at the database level** by two partial Postgres `EXCLUDE` constraints (migration `20260830094335_add_session_overlap_exclusion`, `btree_gist` extension) on `(patient_id, tsrange(start_time, end_time))` and `(team_member_id, tsrange(start_time, end_time))`, scoped to non-excluded statuses — this is the actual concurrency-safety mechanism (see §7); the `findFirst` check alone was never sufficient under concurrent requests.
...
11. **Google failure never invalidates a scheduled session (MEET-01, revised MEET-02 2026-09-01):** a `TherapySession` is valid, scheduled, and committed independently of Google. `sessionMeetingService` never throws for a *Google-side* failure — it records a `FAILED`/`CANCEL_FAILED` state and resolves — and `therapySessionsService.attachMeeting` catches anything else (its Prisma calls can still throw if the database is unreachable) so no integration error can surface as a failed booking. Correspondingly, a Google **cancellation** failure never undoes a Numa cancellation. The one deliberate exception is `deleteSession`, which *refuses* rather than proceeding — see invariant 13. Verified in `sessionMeetingService.test.ts` and the MEET-01/MEET-02 blocks of `therapySessionsService.test.ts`.
12. **One Numa session maps to at most one Google Calendar event (MEET-01, strengthened MEET-02 2026-09-01):** the event id is *derived* from the session id (`numasession<id>`), so a re-attempt after a failed database write-back collides with the existing event (409) and re-adopts it rather than creating a second appointment — this is what makes the create step re-runnable at all. Two further layers protect the database record: `provisionSessionMeeting` returns early when `googleEventId` is set, and the write-back is a compare-and-swap (`updateMany` on `{ id, googleEventId: null }`) backed by a `UNIQUE` index on `therapy_sessions.google_event_id`. **Honest limit:** Google documents that it "cannot guarantee that ID collisions will be detected at event creation time", so this is a strong safeguard, not an absolute one. The compare-and-swap loser no longer deletes its event — since MEET-02 both racers converge on the *same* id, and deleting it would cancel the real meeting; it only discards a genuinely different event, which can happen solely against a session provisioned before MEET-02 with a random Google-assigned id.
13. **A session is never hard-deleted while its calendar event is still live (MEET-02), added 2026-09-01:** the session row is the only place `googleEventId` lives, so deleting it while the event exists would strand a real appointment on the patient's and therapist's calendars, unrecoverable and invisible to the clinic. `deleteSession` therefore inspects the result of `cancelSessionMeeting` and rejects with `409` when an event id survives — including when the cancellation attempt itself errored, since an unknown state is not a safe basis for a destructive operation. This is the only place a Google failure blocks a Numa operation, and it blocks a *destructive* one, which is the conservative direction; it does not weaken invariant 11, which protects sessions being created.

14. **Clinic wall-clock time is the only reference frame for scheduling (TZ-01), added 2026-09-01:** Numa is single-clinic and every date/time an admin enters or reads means **clinic** time (`Asia/Kolkata`, `CLINIC_TIME_ZONE`). The schema mixes two time models — `therapy_sessions.start_time`/`end_time` are absolute instants (`TIMESTAMP(3)`, round-tripped by Prisma as UTC), while `TherapistAvailability.startTime`/`endTime` are TEXT wall clocks and `dayOfWeek` is a clinic weekday — so *something* must convert between them. That conversion happens in exactly one place, `backend/src/lib/clinicTime.ts`, and never implicitly. Two prohibitions follow, and violating either silently reintroduces the bug: **never** build a session instant from a bare datetime string (`new Date("2026-09-15T10:00:00")` is server-local), and **never** read a session instant with `getHours()`/`getDay()`/`getFullYear()`/`toDateString()`/bare `toLocaleTimeString()` (all server-local). The frontend mirrors this for display in `frontend/src/lib/clinicTime.ts`, so what a staff member sees does not depend on their browser's timezone either. Setting `TZ` on the host is fine operationally but is never the correctness mechanism. Verified in `backend/src/lib/__tests__/clinicTime.test.ts`, which runs the conversions in child processes under `TZ=UTC`, `TZ=Asia/Kolkata` and `TZ=America/New_York` and asserts one identical instant.
...
- **No automated test suite exists for most of the codebase.** As of 2026-08-30, `backend/src/services/__tests__/` (vitest, `npm test` in `backend/`) covers the Patient Lifecycle boundary, Capability 1–4 (SCH-05, SCH-04/20, CLN-07, PAT-10), and RBAC's `clinical_notes:sign` permission. Two kinds of coverage now exist: fast tests against an in-memory Prisma double (`fakePrisma.ts`), and `*.integration.test.ts` files that run against a real local Postgres database (`backend/.env`'s `DATABASE_URL`, a disposable local dev/test DB — never Supabase production) — added specifically because the SCH-05 concurrency fix and CLN-07's compare-and-swap sign transaction can only be meaningfully proven against real Postgres locking/constraint behavior, not a fake double. Integration test files skip gracefully (not a hard failure) if that database isn't reachable, and `vitest.config.ts` sets `fileParallelism: false` so they don't race each other on the shared tables. Frontend still has no automated tests; `sessionType` propagation remains covered only at the level the pre-existing suite already covered it.
...
- ~~Scheduling concurrency risk: the check-then-act overlap detection is subject to a race condition under concurrent booking requests for the same therapist/slot.~~ **Resolved 2026-08-30 (SCH-05)** — see §6.4, §7.
...
- **Deterministic Google event ids derived from the session id (MEET-02, 2026-09-01).** Numa’s database was originally the only record that an external calendar event existed, so any path that lost or discarded `googleEventId` while the event was live orphaned a real appointment permanently. Deriving the id from the session id (`numasession<id>`) makes the external event re-discoverable without a stored reference, which is what turns a failed database write-back from permanent into retryable. Format is dictated by Google: base32hex only (lowercase `a`-`v` and digits, 5-1024 chars), which rules out hyphens — "numa-session-1" would be rejected outright. Rejected alternative: a reconciliation job or an orphaned-events table, both of which add infrastructure to solve a problem the id format solves for free. Backward compatible: sessions provisioned before this change keep their random Google-assigned ids and are never re-inserted.
...
- **One clinic timezone, converted explicitly, no date library (TZ-01, 2026-09-01).** Scheduling input is clinic wall-clock; the database holds instants; `backend/src/lib/clinicTime.ts` is the only bridge. Alternatives rejected: relying on `TZ=Asia/Kolkata` in Render (a deployment setting silently becomes a correctness dependency, and local/dev/test environments would disagree); storing local date + time columns instead of instants (a schema rewrite, and it would break the existing `EXCLUDE` overlap constraints which operate on `tsrange`); adding Luxon or date-fns-tz (`Intl.DateTimeFormat` with an IANA `timeZone` does the whole job in ~30 lines, DST-aware, and Node >= 20 ships full ICU — consistent with the `googleapis` decision above). A clinic-settings domain or per-user timezones were explicitly out of scope: multi-timezone scheduling is a SPEC non-goal.
